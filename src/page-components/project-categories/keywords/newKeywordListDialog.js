...
const NewKeywordListDialog = ({ isOpen, closeModal }) => {
  ...
  const continueKeywordCreation = () => {
    router.push('/app/projects/keywords')
  }

  return (
    <DialogLayout isSharp={true} widthRestrict={'max-w-[1299px]'} isOpen={isOpen} closeModal={closeModal}>
      <div className='w-full text-left pt-[30px] divide-y-[1px] dark:divide-darkMode-border divide-ash'>
        ...
        <div className='form-group px-14 py-4 flex mb-0 justify-between'>
          ...
          <div className="space-x-4 flex">
            <button type='button' onClick={closeModal} className="btn btn-reset dark:text-white text-black">
              Cancel
            </button>
            <button type='button' onClick={continueKeywordCreation} className='block w-fit btn btn-primary bg-primary text-white'>
              Continue
            </button>
          </div>
        </div>
