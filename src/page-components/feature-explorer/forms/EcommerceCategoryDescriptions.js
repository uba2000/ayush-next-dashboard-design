...
const EcommerceCategoryDescriptions = ({ explorerState, dispatch, generate }) => {
  return (
    <FormLayout
    ...
    >
      ...
      <ExplorerTwoInputLayout>
        <FormGroup label={<LabelLayout>Business Name:</LabelLayout>} labelFor={'productName'}>
          <Input
            value={explorerState.businessName}
            onChange={(e) => dispatch({ value: e.target.value, type: 'setBusinessName' })}
            id={'productName'}
      ...
      <FormGroup label={<LabelLayout>Business Description:</LabelLayout>} labelFor={'subHeading'} className="mb-4">
        <textarea
          value={explorerState.businessDescription}
          onChange={(e) => dispatch({ value: e.target.value, type: 'setBusinessDescription' })}
          id={'keywords'}
      ...
      <ExplorerTwoInputLayout>
        <FormGroup label={<LabelLayout>Category Name:</LabelLayout>} labelFor={'categoryName'}>
          <Input
            value={explorerState.categoryName}
            onChange={(e) => dispatch({ value: e.target.value, type: 'setCategoryName' })}
            id={'categoryName'}
            placeholder="Shoes,clothing"
            className="rounded"
          />
        </FormGroup>
        <FormGroup label={<LabelLayout>Keyword:</LabelLayout>} labelFor={'keywordInput'}>
          <Input
